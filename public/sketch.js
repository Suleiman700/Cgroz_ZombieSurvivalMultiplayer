...
socket.on('play bullet sound', function (data) {
    const currentGunName = data['currentGunName']
    const BulletSound = new GunsSounds()
    BulletSound.play(currentGunName)
});
...
function setup() {
    ...
    wallGuns = [
        this.topGunPickup = new WallGun(150, -40, new AK(clientPlayer.x, clientPlayer.y)),
        // this.topGunPickup = new WallGun(350, -40, new AK(clientPlayer.x, clientPlayer.y)),
    ]
...
function draw() {
    ...
    if (!gameActive && !sessionOver) {
        ...
        if ((keyIsDown(82) && !score.reloading) || score.ammoIn == 0 && !score.reloading) {
            ...
            const BulletSound = new GunsSounds()
            BulletSound.play_reload(currentGun.name)
        }
...
function sendBulletData() {
    let bulletData = {
        ...
        shooter_player_pos_x: clientPlayer['pos_x'],
        shooter_player_pos_y: clientPlayer['pos_y'],
